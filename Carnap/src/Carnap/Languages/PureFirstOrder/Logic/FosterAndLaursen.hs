...
module Carnap.Languages.PureFirstOrder.Logic.FosterAndLaursen 
    ( fosterAndLaursenFOLCoreCalc, fosterAndLaursenFOLCalc, parseFosterAndLaursenFOL, FosterAndLaursenFOL
    , fosterAndLaursenFOL2019Calc, parseFosterAndLaursenFOLCore, FosterAndLaursenFOLCore
    , fosterAndLaursenFOLPlus2019Calc
    ) where
...
import qualified Carnap.Languages.PurePropositional.Logic.FosterAndLaursen as TFL
...
data FosterAndLaursenFOLCore = TFLC P.FosterAndLaursenTFLCore | UI | UE | EI | EE1 | EE2 | IDI | IDE1 | IDE2 
                            | Pr (Maybe [(ClassicalSequentOver PureLexiconFOL (Sequent (Form Bool)))])
                    deriving Eq

data FosterAndLaursenFOL = TFL P.FosterAndLaursenTFL | FOL FosterAndLaursenFOLCore | QN1 | QN2 | QN3 | QN4

instance Show FosterAndLaursenFOLCore where
        show (TFLC x) = show x
        show UI          = "∀I"
        show UE          = "∀E"
        show EI          = "∃I"
...
instance Show FosterAndLaursenFOL where
        show (TFL x)     = show x
        show (FOL x)     = show x
        show QN1         = "CQ"
        show QN2         = "CQ"
...
instance Inference FosterAndLaursenFOLCore PureLexiconFOL (Form Bool) where
...
instance Inference FosterAndLaursenFOL PureLexiconFOL (Form Bool) where
...
parseFosterAndLaursenFOLCore rtc = try quantRule <|> (map TFLC <$> parseProp)
    where parseProp = P.parseFosterAndLaursenTFLCore (defaultRuntimeDeductionConfig)
...
parseFosterAndLaursenFOL rtc = try premRule <|> try (map TFL <$> parseProp) <|> try (map FOL <$> quantRule) <|>  try cqRule
    where parseProp = P.parseFosterAndLaursenTFL (defaultRuntimeDeductionConfig)
          quantRule = parseFosterAndLaursenFOLCore rtc
...
fosterAndLaursenFOLCoreCalc = mkNDCalc
    { ndRenderer = FitchStyle StandardFitch
...
fosterAndLaursenFOL2019Calc = mkNDCalc
    { ndRenderer = FitchStyle StandardFitch
...
fosterAndLaursenFOLPlus2019Calc = mkNDCalc
    { ndRenderer = FitchStyle StandardFitch
